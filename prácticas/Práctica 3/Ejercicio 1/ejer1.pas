...
procedure leer(var arch:arch_emp; var reg:empleado);
begin
...
procedure eliminar(var arch:arch_emp);
var
	reg:empleado;
	num:integer;
	i:integer;
begin
	...
	readln(num);
	leer(arch,reg);
	//Avanzo hasta encontrar el registro o llegar al final
	while( (reg.numero <> valorAlto) and (reg.numero<>num) )do
		leer(arch,reg);
		
	//Si encontre el registro (no llegue a EOF)
	if(reg.numero<>valorAlto)then
	begin
		i:=filepos(arch);//Guardo el indice del registro a borrar
		seek(arch,filesize(arch)-1);//Voy al final del archivo
		read(arch,reg);//Leo el archivo
		seek(arch,i-1);//Me posiciono en el registro a reemplazar
		write(arch,reg);//Escribo el registro
		seek(arch,filesize(arch)-1);//Vuelvo al final del archivo y lo trunco
		truncate(arch);
		writeln('Empleado numero: ',num,' eliminado correctamente');
	end
	else
		writeln('Empleado numero: ',num,' no encontrado');
